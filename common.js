...
function term_command(s) {
	...
	if (cmd2 == 'wq' || cmd == 'x') {
		...
		filepicker.getFile('*/*', {'modal': true}, function (url, data) {
			term.filePickerHandle = url;
			$.ajax({url: url,
				success: function(data) {
					term._formelement.value = data;
					term_thaw(term._formelement.value);
					term_redraw();
				}});
		});
...
function editor_disable(sav) {
	self.close(); //close the window
        statustext = "Can't close the window";
        term_redraw();
...
//	}
	...
	//load from filepicker
	var f = gup('f');
	if (f) {
		term.filePickerHandle = f;
		$.ajax({url: f,
			success: function(data) {
				term._formelement.value = data;
				term_thaw(term._formelement.value);
...
if(mw.config.get('wgAction') === 'edit') {
     editor(document.forms[0].elements.wpTextbox1);
}
